...
set "THIRD_PARTY_NOTICES_SOURCE=%SOURCE_ROOT%\THIRD_PARTY_NOTICES.md"
...
if not exist "%THIRD_PARTY_NOTICES_SOURCE%" (
    echo ERROR: Third-party notices were not found:
    echo   %THIRD_PARTY_NOTICES_SOURCE%
    exit /b 1
)
...
if not exist "%BUILD_DIR%\libmcut.dll" (
    echo ERROR: The MCUT runtime was not found beside BrickSuite.exe:
    echo   %BUILD_DIR%\libmcut.dll
    exit /b 1
)

if not exist "%BUILD_DIR%\deployment\licenses\MCUT\LICENSE.txt" (
    echo ERROR: The staged MCUT license was not generated by CMake.
    exit /b 1
)
if not exist "%BUILD_DIR%\deployment\licenses\MCUT\COPYING.LESSER" (
    echo ERROR: The staged MCUT LGPL terms were not generated by CMake.
    exit /b 1
)
...
echo Copying third-party notices and MCUT runtime...
copy /Y "%THIRD_PARTY_NOTICES_SOURCE%" "%STAGE_DIR%\THIRD_PARTY_NOTICES.md" >nul
if errorlevel 1 (
    echo ERROR: Unable to stage third-party notices.
    exit /b 1
)
copy /Y "%BUILD_DIR%\libmcut.dll" "%STAGE_DIR%\libmcut.dll" >nul
if errorlevel 1 (
...
call :checkfile "THIRD_PARTY_NOTICES.md"
