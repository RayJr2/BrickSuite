    ...
    const RebrickableImportCancellation* token, const RebrickableRowProgress& progress)
{
    ...
    while (!input.stream.atEnd()) {
        ...
        if (!isSet) { ++result.recognizedMinifigInventories; continue; }
    ...
    if(commit(database,result)) m_inventoryOwners=owners;
...
RebrickableInventoryCompositionImporter::Result importChildren(
    ...
    const RebrickableImportCancellation* token, const RebrickableRowProgress& progress,
    const QHash<QString,bool>& owners)
{
    ...
    if(!temporary.exec("CREATE TEMP TABLE IF NOT EXISTS current_inventory_child(revision_id INTEGER,identity_id INTEGER,color_id INTEGER,spare INTEGER,PRIMARY KEY(revision_id,identity_id,color_id,spare))")
        ||!temporary.exec("DELETE FROM current_inventory_child")){result.message=temporary.lastError().text();database.rollback();return result;}
    QSqlQuery seen(database);seen.prepare("INSERT INTO current_inventory_child(revision_id,identity_id,color_id,spare) VALUES(:revision,:identity,:color,:spare)");
    QSqlQuery insert(database),update(database);
    ...
    while(!input.stream.atEnd()){
        ...
        if(ownerIt!=owners.constEnd()&&!ownerIt.value()){
            ...
            ++result.ignoredMinifigPartRows;continue;
        }
        ...
        seen.bindValue(":revision",revision);seen.bindValue(":identity",identityIt.value());seen.bindValue(":color",kind==ChildKind::Parts?colorIt.value():0);seen.bindValue(":spare",kind==ChildKind::Parts?spare:false);
    ...
    QString cleanupSql="DELETE FROM "+table+" WHERE set_inventory_revision_id=:revision AND NOT EXISTS (SELECT 1 FROM current_inventory_child c WHERE c.revision_id="+table+".set_inventory_revision_id AND c.identity_id="+table+"."+identityColumn;
    ...
    commit(database,result);qInfo()<<expected<<"import elapsed ms:" <<timer.elapsed()<<"rows:" <<result.rowsRead<<"Set-inventory rows:" <<result.setInventoryRows<<"Minifig-owned rows ignored:" <<result.ignoredMinifigPartRows<<"replaced:" <<result.replaced;return result;
}
}

RebrickableInventoryCompositionImporter::Result RebrickableInventoryCompositionImporter::importParts(const QString& f,QSqlDatabase& d,const RebrickableImportCancellation* c,const RebrickableRowProgress& p){return importChildren(ChildKind::Parts,f,d,c,p,m_inventoryOwners);}
RebrickableInventoryCompositionImporter::Result RebrickableInventoryCompositionImporter::importMinifigs(const QString& f,QSqlDatabase& d,const RebrickableImportCancellation* c,const RebrickableRowProgress& p){return importChildren(ChildKind::Minifigs,f,d,c,p,m_inventoryOwners);}
RebrickableInventoryCompositionImporter::Result RebrickableInventoryCompositionImporter::importSets(const QString& f,QSqlDatabase& d,const RebrickableImportCancellation* c,const RebrickableRowProgress& p){return importChildren(ChildKind::Sets,f,d,c,p,m_inventoryOwners);}
