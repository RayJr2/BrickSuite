    ...
    AsyncReadCompletion<RemoteReadDto::Page<RemoteReadDto::InventoryRow>> completion)
{
    QJsonObject payload{{"workspaceId",double(r.workspaceId)},{"text",r.text},{"storageId",double(r.storageId)},
        {"rebrickableColorId",r.rebrickableColorId},{"page",r.paging.page},{"pageSize",r.paging.pageSize}};
...
ReadRequestToken RemoteReadApplicationServices::getBuild(qint64 id,QObject*context,AsyncReadCompletion<RemoteReadDto::BuildDetail> completion)
{return request<RemoteReadDto::BuildDetail>("builds.get",{{"buildId",double(id)}},context,std::move(completion),[](const QJsonObject&o,auto*out,QString*error){RemoteReadJson::DecodeError e;const auto v=o.value("build");if(!v.isObject()||!RemoteReadJson::fromJson(v.toObject(),static_cast<RemoteReadDto::BuildSummary*>(out),&e)){if(error)*error=e.message;return false;}return true;});}

ReadRequestToken RemoteReadApplicationServices::getInventory(qint64 id,QObject*context,AsyncReadCompletion<RemoteReadDto::InventoryDetail> completion)
{return request<RemoteReadDto::InventoryDetail>("inventory.get",{{"inventoryRecordId",double(id)}},context,std::move(completion),[](const QJsonObject&o,auto*out,QString*error){RemoteReadJson::DecodeError e;const auto v=o.value("item");if(!v.isObject()||!RemoteReadJson::fromJson(v.toObject(),out,&e)){if(error)*error=e.message;return false;}return true;});}
...
QJsonObject buildPagePayload(qint64 id,const RemoteReadDto::PageRequest&p){return{{"buildId",double(id)},{"page",p.page},{"pageSize",p.pageSize}};}
}
ReadRequestToken RemoteReadApplicationServices::buildRequirements(qint64 id,const RemoteReadDto::PageRequest&p,QObject*context,AsyncReadCompletion<RemoteReadDto::Page<RemoteReadDto::BuildRequirement>> completion)
{return request<RemoteReadDto::Page<RemoteReadDto::BuildRequirement>>("builds.requirements",buildPagePayload(id,p),context,std::move(completion),[](const auto&o,auto*out,QString*e){return decodePage(o,out,e,[](const auto&j,auto*x,auto*de){return RemoteReadJson::fromJson(j,x,de);});});}
...
{auto payload=buildPagePayload(id,p);payload["workspaceId"]=double(workspace);return request<RemoteReadDto::Page<RemoteReadDto::MissingPart>>("builds.missingParts",payload,context,std::move(completion),[](const auto&o,auto*out,QString*e){return decodePage(o,out,e,[](const auto&j,auto*x,auto*de){return RemoteReadJson::fromJson(j,x,de);});});}
ReadRequestToken RemoteReadApplicationServices::pulling(qint64 id,const RemoteReadDto::PageRequest&p,QObject*context,AsyncReadCompletion<RemoteReadDto::Page<RemoteReadDto::PullingRow>> completion)
{return request<RemoteReadDto::Page<RemoteReadDto::PullingRow>>("builds.pulling",buildPagePayload(id,p),context,std::move(completion),[](const auto&o,auto*out,QString*e){return decodePage(o,out,e,[](const auto&j,auto*x,auto*de){return RemoteReadJson::fromJson(j,x,de);});});}
