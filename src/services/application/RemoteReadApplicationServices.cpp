...
ReadRequestToken RemoteReadApplicationServices::collectionDisassemblyPlan(qint64 workspace,qint64 id,QObject*context,AsyncReadCompletion<RemoteReadDto::CollectionDisassemblyPlan> completion)
{return request<RemoteReadDto::CollectionDisassemblyPlan>("collection.disassemblyPlan",{{"workspaceId",double(workspace)},{"collectionItemId",double(id)}},context,std::move(completion),[](const QJsonObject&o,auto*out,QString*error){const auto value=o.value("plan");RemoteReadJson::DecodeError e;if(!value.isObject()||!RemoteReadJson::fromJson(value.toObject(),out,&e)){if(error)*error=e.message;return false;}return true;});}
...
ReadRequestToken RemoteReadApplicationServices::resolvePickABrickPart(const QString&part,int color,QObject*context,AsyncReadCompletion<RemoteReadDto::PickABrickPartResolution> completion)
{return request<RemoteReadDto::PickABrickPartResolution>("parts.pickABrick.resolve",{{"partNumber",part.trimmed()},{"rebrickableColorId",color}},context,std::move(completion),[](const QJsonObject&o,auto*out,QString*error){const auto value=o.value("resolution");RemoteReadJson::DecodeError e;if(!value.isObject()||!RemoteReadJson::fromJson(value.toObject(),out,&e)){if(error)*error=e.message;return false;}return true;},true);}
