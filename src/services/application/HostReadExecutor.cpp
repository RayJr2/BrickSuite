...
#include "../inventory/InventoryExportService.h"
#include "../collection/CollectionExportService.h"
...
void HostReadExecutor::exportInventoryPortable(const RemoteReadDto::InventorySearchRequest&r,QObject*context,std::function<void(const RemoteReadDto::Page<RemoteReadDto::InventoryExportRow>&)>completion,ErrorCallback failure)
{
 QPointer<QObject>guard(context);enqueue(QStringLiteral("inventory.export"),[=,completion=std::move(completion)](ApplicationServices&s,const QSqlDatabase&db)mutable{InventorySearchCriteria c;c.workspaceId=int(r.workspaceId);c.searchText=r.text;c.storageLocationId=int(r.storageId);c.limit=r.paging.pageSize;c.offset=(r.paging.page-1)*r.paging.pageSize;if(r.rebrickableCategoryId>=0){const auto category=PartCategoryRepository(db).getByRebrickableId(r.rebrickableCategoryId);if(!category){RemoteReadDto::Page<RemoteReadDto::InventoryExportRow> empty;empty.page=r.paging.page;empty.pageSize=r.paging.pageSize;if(guard)QMetaObject::invokeMethod(guard,[guard,completion,empty]()mutable{if(guard)completion(empty);},Qt::QueuedConnection);return;}c.categoryId=category->id();}if(r.rebrickableColorId>=0){const auto color=ColorRepository(db).getByRebrickableId(r.rebrickableColorId);if(!color){RemoteReadDto::Page<RemoteReadDto::InventoryExportRow> empty;empty.page=r.paging.page;empty.pageSize=r.paging.pageSize;if(guard)QMetaObject::invokeMethod(guard,[guard,completion,empty]()mutable{if(guard)completion(empty);},Qt::QueuedConnection);return;}c.colorId=color->id();}auto page=s.inventory().search(c);for(auto&source:page.rows){const QString path=locationPath(db,source.storageLocationId);if(!path.isEmpty())source.storageLocationName=path;}RemoteReadDto::Page<RemoteReadDto::InventoryExportRow> out;out.page=r.paging.page;out.pageSize=r.paging.pageSize;out.totalRows=page.total;const QSet<QString> requested(r.exportFields.begin(),r.exportFields.end());for(const auto&row:InventoryExportService(db).createRows(page.rows,requested)){RemoteReadDto::InventoryExportRow v;v.inventoryRecordId=row.inventoryRecordId;v.partNumber=row.partNumber;v.partName=row.partName;v.category=row.category;v.color=row.color;v.quantity=row.quantity;v.storagePath=row.storagePath;v.manufacturer=row.manufacturer;v.condition=row.condition;v.ownership=row.ownership;v.legoElementIds=row.legoElementIds;v.rebrickablePartId=row.rebrickablePartId;v.rebrickableColorId=row.rebrickableColorId;v.brickLinkPartIds=row.brickLinkPartIds;v.brickLinkColorId=row.brickLinkColorId;out.rows.append(v);}if(guard)QMetaObject::invokeMethod(guard,[guard,completion,out=std::move(out)]()mutable{if(guard)completion(out);},Qt::QueuedConnection);},context,std::move(failure));
}

void HostReadExecutor::exportCollectionPortable(const CollectionSearchCriteria&criteria,QObject*context,std::function<void(const RemoteReadDto::Page<RemoteReadDto::CollectionExportRow>&)>completion,ErrorCallback failure)
{
 QPointer<QObject>guard(context);enqueue(QStringLiteral("collection.export"),[=,completion=std::move(completion)](ApplicationServices&s,const QSqlDatabase&db)mutable{const auto page=s.collection().search(criteria);RemoteReadDto::Page<RemoteReadDto::CollectionExportRow>out;out.page=criteria.offset/criteria.limit+1;out.pageSize=criteria.limit;out.totalRows=page.total;auto source=page.rows;for(auto&x:source)if(x.item.storageLocationId>0)x.locationName=locationPath(db,x.item.storageLocationId);for(const auto&r:CollectionExportService::createRows(source)){RemoteReadDto::CollectionExportRow v;v.collectionItemId=r.collectionItemId;v.type=r.type;v.reference=r.reference;v.name=r.name;v.nickname=r.nickname;v.state=r.state;v.condition=r.condition;v.completeness=r.completeness;v.location=r.location;v.source=r.source;v.allowPartsSource=r.allowPartsSource;v.notes=r.notes;v.sourceBuildReference=r.sourceBuildReference;v.sourceBuildName=r.sourceBuildName;v.createdUtc=r.createdUtc;v.modifiedUtc=r.modifiedUtc;out.rows<<v;}if(guard)QMetaObject::invokeMethod(guard,[guard,completion,out=std::move(out)]()mutable{if(guard)completion(out);},Qt::QueuedConnection);},context,std::move(failure));
}
