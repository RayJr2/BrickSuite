...
FunctionalOperandRegenerationResult FunctionalOperandRegenerator::regenerateStud(const FunctionalFeature&feature,MaleStudDimensionalCorrection correction)
{
    ...
    if(feature.family!=FunctionalInterfaceFamily::StandardStud||feature.role!=FunctionalInterfaceRole::Male||feature.materialSide!=FunctionalMaterialSide::MaterialInside||feature.operandAction!=FunctionalOperandAction::Unite||feature.eligibility!=FunctionalEligibility::Eligible||feature.confidence!=SemanticConfidence::HighConfidence||feature.constructionRecipe!=QStringLiteral("standard-solid-stud-v1")){
        result.error=FunctionalOperandRegenerationError::UnsupportedFeature;result.diagnostic=QStringLiteral("The functional feature is not an eligible standard solid stud contract.");return result;}
    ...
    for(const auto&section:result.executedProfile)for(int i=0;i<RoundProfileSegments;++i){const double angle=2.0*pi*double(i)/double(RoundProfileSegments),cs=std::cos(angle),sn=std::sin(angle);const Point radial{frame.profileU.x*cs+frame.profileV.x*sn,frame.profileU.y*cs+frame.profileV.y*sn,frame.profileU.z*cs+frame.profileV.z*sn};mesh.vertices.push_back({frame.origin.x+frame.axis.x*section.axialPositionMillimetres+radial.x*section.radiusMillimetres,frame.origin.y+frame.axis.y*section.axialPositionMillimetres+radial.y*section.radiusMillimetres,frame.origin.z+frame.axis.z*section.axialPositionMillimetres+radial.z*section.radiusMillimetres});}
    for(int i=0;i<RoundProfileSegments;++i){const int next=(i+1)%RoundProfileSegments;mesh.faces.push_back({std::uint32_t(i),std::uint32_t(next),std::uint32_t(RoundProfileSegments+i)});mesh.faces.push_back({std::uint32_t(next),std::uint32_t(RoundProfileSegments+next),std::uint32_t(RoundProfileSegments+i)});}const auto bottom=std::uint32_t(mesh.vertices.size());mesh.vertices.push_back({frame.origin.x-frame.axis.x*intrusion,frame.origin.y-frame.axis.y*intrusion,frame.origin.z-frame.axis.z*intrusion});const auto top=std::uint32_t(mesh.vertices.size());mesh.vertices.push_back({frame.origin.x+frame.axis.x*result.resultingAxialExtentMillimetres,frame.origin.y+frame.axis.y*result.resultingAxialExtentMillimetres,frame.origin.z+frame.axis.z*result.resultingAxialExtentMillimetres});for(int i=0;i<RoundProfileSegments;++i){const auto next=std::uint32_t((i+1)%RoundProfileSegments);mesh.faces.push_back({bottom,next,std::uint32_t(i)});mesh.faces.push_back({top,std::uint32_t(RoundProfileSegments+i),std::uint32_t(RoundProfileSegments)+next});}
    normalize(&mesh);result.analysis=analyzeSource(mesh);const auto validation=validateBooleanOperand(result.analysis);if(!validation.ok()){result.error=FunctionalOperandRegenerationError::InvalidGeneratedOperand;result.diagnostic=QStringLiteral("The regenerated standard stud failed validation: %1").arg(QString::fromStdString(validation.message));return result;}result.error=FunctionalOperandRegenerationError::None;result.diagnostic=QStringLiteral("Standard stud regenerated with independent radial and axial corrections.");return result;
}
