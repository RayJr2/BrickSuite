...
bool dispatchUnion(const PrintMesh&a,const PrintMesh&b,PrintMesh*out,std::string*error)
{
    ...
    const McFlags flags=MC_DISPATCH_VERTEX_ARRAY_DOUBLE|MC_DISPATCH_ENFORCE_GENERAL_POSITION|MC_DISPATCH_FILTER_FRAGMENT_SEALING_OUTSIDE|MC_DISPATCH_FILTER_FRAGMENT_LOCATION_ABOVE;
    ...
    if(status!=MC_NO_ERROR){mcReleaseContext(context);*error="MCUT union dispatch failed ("+std::to_string(int(status))+")";return false;}
    McUint32 count=0;status=mcGetConnectedComponents(context,MC_CONNECTED_COMPONENT_TYPE_FRAGMENT,0,nullptr,&count);
    if(status!=MC_NO_ERROR||count==0){mcReleaseContext(context);*error="MCUT union returned no fragments";return false;}
...
MeshBooleanResult McutMeshBooleanService::unite(const PrintMesh&source,const PrintMesh&additive)
{
    ...
    if(!dispatchUnion(source,additive,&r.mesh,&r.message)){r.error=MeshBooleanError::BackendFailure;return r;}
