...
PrintPreparationResult LDrawPrintPreparationService::prepare(const PrintPreparationRequest&request,const CancellationState*cancellation,const PrintPreparationProgressCallback&progress)const
{
    ...
    try{
        ...
        if(std::size_t(operands.size()-1)>request.profile.maximumBooleanOperations){
        ...
        PrintPreparationResult result;result.sourceAnalysis=sourceAnalysis;result.sourceCoverage=semantic.coverage;result.semanticOperandCount=std::size_t(operands.size());result.preparationProfileIdentity=request.profile.identity;PrintMesh accumulated=operands.front().closedMesh;phase.restart();
