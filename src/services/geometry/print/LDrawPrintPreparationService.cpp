...
{
    if(!source.bounds.valid||source.triangles>2000||source.connectedComponents<2||source.connectedComponents>256||source.boundaryEdges>2048)return false;auto d=dimensions(source.bounds);std::array<double,3>sorted{d.x,d.y,d.z};std::sort(sorted.begin(),sorted.end());
    ...
    const bool click=source.triangles<=1600 && source.boundaryEdges>=16 &&
        ClickHingeSemantic::recognize(loadResult).size()==1;
    if(click)return sorted[0]<=12.0&&sorted[1]<=16.0&&sorted[2]<=32.0;
    // Only the certified compact notched wheel bearing is admitted here.
    const auto wheels=RetainedRotatingWheelSemantic::recognize(loadResult);
...
PrintPreparationResult LDrawPrintPreparationService::prepare(const PrintPreparationRequest&request,const CancellationState*cancellation,const PrintPreparationProgressCallback&progress)const
{
    ...
    try{
        ...
        if(!semantic.coverage.complete()){
            ...
            auto rejected=failure(PrintPreparationError::OperandValidationFailed,sourceAnalysis,request.profile.identity,
                QStringLiteral("Authoritative source coverage is incomplete; uncovered groups: %1.").arg(groups.join(',')));
        ...
        prepared->sourceAnalysis=sourceAnalysis;prepared->finalAnalysis=finalAnalysis;prepared->sourceCoverage=semantic.coverage;
        ...
        for(const auto& hinge:ClickHingeSemantic::recognize(request.loadResult)) {
            const bool retained=std::any_of(prepared->functionalFeatures.cbegin(),prepared->functionalFeatures.cend(),
                [&](const FunctionalFeature& feature){return feature.stableIdentity==hinge.stableIdentity;});
            if(!retained)prepared->functionalFeatures.push_back(hinge);
        }
        ...
        result.finalAnalysis=finalAnalysis;result.timings=timings;result.diagnostic=localComposition.successful?
            localComposition.diagnostic+QStringLiteral(" Nominal PreparedMesh independently validated."):
            QStringLiteral("Nominal PreparedMesh independently validated.");
