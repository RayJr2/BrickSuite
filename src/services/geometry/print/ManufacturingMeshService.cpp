...
const FitProfileCorrection* ManufacturingMeshService::compatibleCorrection(const FitProfile&profile,FitPrintedOrientation orientation,QString*reason){QString compatibility;if(!FitCalibrationLibrary::profileCompatibility(profile,&compatibility)){if(reason)*reason=compatibility;return nullptr;}const auto correction=std::find_if(profile.corrections.cbegin(),profile.corrections.cend(),[&](const auto&c){return c.featureFamily=="RoundTechnicPassage"&&c.featureRole=="female"&&c.printedOrientation==orientationName(orientation)&&c.units=="millimetres"&&c.semantics=="female-diameter-clearance";});if(correction==profile.corrections.cend()){if(reason)*reason="The selected profile has no compatible female RoundTechnicPassage correction for this print orientation.";return nullptr;}if(reason)*reason="Compatible";return &*correction;}
ManufacturingMeshResult ManufacturingMeshService::generate(const LDrawGeometry::LDrawLoadResult&source,const PreparedMesh&prepared,const FitProfile&profile,FitPrintedOrientation orientation)const{
    ...
    const auto correction=compatibleCorrection(profile,orientation,&reason);
    if(!correction)return fail(ManufacturingMeshError::MissingCorrection,reason);
