...
bool correctionApplies(const FunctionalFeature&feature,const ManufacturingMeshCorrections&corrections){return (feature.family==FunctionalInterfaceFamily::RoundTechnicPassage&&feature.role==FunctionalInterfaceRole::Female&&corrections.femaleDiameter&&feature.evidenceContract==corrections.femaleDiameter->semanticContractVersion)||(feature.family==FunctionalInterfaceFamily::StandardStud&&feature.role==FunctionalInterfaceRole::Male&&((corrections.studDiameter&&feature.evidenceContract==corrections.studDiameter->semanticContractVersion)||(corrections.studHeight&&feature.evidenceContract==corrections.studHeight->semanticContractVersion)));}
...
ManufacturingMeshCorrections ManufacturingMeshService::compatibleCorrections(const FitProfile&profile,FitPrintedOrientation orientation,QString*reason)
{
    ...
    for(const auto&correction:profile.corrections){if(matches(correction,"RoundTechnicPassage","female","female-diameter-clearance",printedOrientation))result.femaleDiameter=&correction;else if(matches(correction,"StandardStud","male","male-stud-diameter",printedOrientation))result.studDiameter=&correction;else if(matches(correction,"StandardStud","male","male-stud-height",printedOrientation))heightCandidate=&correction;}
...
{
    const auto corrections=compatibleCorrections(profile,orientation,reason);return corrections.femaleDiameter?corrections.femaleDiameter:(corrections.studDiameter?corrections.studDiameter:corrections.studHeight);
}
...
{
    return hasApplicableCorrection(profile,LDrawSemanticOperandBuilder::build(source),printOrientation,reason);
}
