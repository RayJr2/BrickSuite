...
struct PrintPreparationResult {PrintPreparationState state=PrintPreparationState::Failed;PrintPreparationError error=PrintPreparationError::BackendFailure;std::shared_ptr<const PreparedMesh> preparedMesh;MeshAnalysisResult sourceAnalysis,finalAnalysis;QVector<BooleanOperationSummary> operations;PrintPreparationTimings timings;DimensionalFidelityResult dimensionalFidelity;SourceCoverage sourceCoverage;QString preparationProfileIdentity,diagnostic;QStringList warnings,detailedDiagnostics;std::size_t semanticOperandCount=0;bool cacheHit=false;bool ready()const{return state==PrintPreparationState::Ready&&preparedMesh;}};
}
