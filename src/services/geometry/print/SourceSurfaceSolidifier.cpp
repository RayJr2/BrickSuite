    ...
    const LDrawGeometry::LDrawLoadResult&source,double pitch)
{
    ...
    for(const auto&vertex:mesh.vertices){Point nearest{};double nearestSquared=std::numeric_limits<double>::max();for(const auto&triangle:triangles){const auto candidate=closestPointOnTriangle(vertex,triangle.a,triangle.b,triangle.c);const double candidateSquared=squaredDistance(vertex,candidate);if(candidateSquared<nearestSquared){nearestSquared=candidateSquared;nearest=candidate;}}const double distance=std::sqrt(nearestSquared);beforeDistance+=distance;projections.push_back({nearest,distance,nearestSquared<=maximumProjectionDistanceSquared});}
