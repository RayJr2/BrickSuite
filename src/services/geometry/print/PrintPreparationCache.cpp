...
quint64 PrintPreparationCache::approximateBytes(const PreparedMesh&m){quint64 n=sizeof(PreparedMesh)+m.mesh.vertices.capacity()*sizeof(Point)+m.mesh.faces.capacity()*sizeof(Face)+m.finalAnalysis.issues.capacity()*sizeof(MeshIssue);for(const auto&s:m.operationSummary)n+=quint64(s.capacity()+1)*2;for(const auto&s:m.warnings)n+=quint64(s.capacity()+1)*2;return n;}
