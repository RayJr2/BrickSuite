...
StandardStudCalibrationArtifactResult StandardStudCalibrationArtifact::generate(const FunctionalFeature&prototype,const StandardStudCalibrationArtifactDefinition&definition)
{
    ...
    PrintMesh body=keyedBase(definition.candidateCount);McutMeshBooleanService booleans;const int half=definition.candidateCount/2;
    for(int i=0;i<definition.candidateCount;++i){const double varied=definition.centerCorrectionMillimetres+(i-half)*definition.candidateSpacingMillimetres;MaleStudDimensionalCorrection correction;correction.diameterMillimetres=definition.dimension==StandardStudCalibrationDimension::Diameter?varied:definition.fixedDiameterCorrectionMillimetres;correction.heightMillimetres=definition.dimension==StandardStudCalibrationDimension::Height?varied:0.0;FunctionalFeature candidate=prototype;candidate.stableIdentity=QStringLiteral("%1:candidate-%2").arg(definition.artifactIdentity).arg(i+1);candidate.frame.origin={8.0+12.0*i,5.0,3.0};candidate.frame.axis={0,0,1};candidate.frame.profileU={1,0,0};candidate.frame.profileV={0,1,0};const auto regenerated=FunctionalOperandRegenerator::regenerateStud(candidate,correction);if(!regenerated.ok()){result.diagnostic=QStringLiteral("Stud candidate %1 could not be regenerated: %2").arg(i+1).arg(regenerated.diagnostic);return result;}const auto united=booleans.unite(body,regenerated.mesh);if(!united.ok()){result.diagnostic=QStringLiteral("Stud candidate %1 could not be joined to the calibration base.").arg(i+1);return result;}body=united.mesh;FitCalibrationCandidate record;record.index=i+1;record.diameterCorrectionMillimetres=correction.diameterMillimetres;record.functionalDiameterMillimetres=regenerated.resultingGoverningRadiusMillimetres*2.0;record.heightCorrectionMillimetres=correction.heightMillimetres;record.functionalHeightMillimetres=regenerated.resultingAxialExtentMillimetres;result.candidates.push_back(record);}
