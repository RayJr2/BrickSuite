...
namespace
{
QString allocationSelectColumns()
{
    return QStringLiteral(
        "id, build_id, build_requirement_id, inventory_record_id, part_id, color_id, "
...
bool BuildAllocationRepository::create(BuildAllocation& allocation)
{
    ...
    query.bindValue(":build_requirement_id",
                    allocation.buildRequirementId() > 0 ? QVariant(allocation.buildRequirementId()) : QVariant());
...
QList<BuildAllocation> BuildAllocationRepository::getByRequirement(int buildRequirementId) const
{
    ...
    if (!query.exec()) {
        qCritical() << "Unable to retrieve requirement allocations:" << query.lastError().text();
        return allocations;
    }
    while (query.next())
        allocations.append(allocationFromQuery(query));
...
QList<BuildAllocation> BuildAllocationRepository::getByInventoryRecord(int inventoryRecordId) const
{
    ...
    if (!query.exec()) {
        qCritical() << "Unable to retrieve inventory build allocations:" << query.lastError().text();
        return allocations;
    }
    while (query.next())
        allocations.append(allocationFromQuery(query));
...
bool BuildAllocationRepository::update(BuildAllocation& allocation)
{
    ...
    query.prepare(R"(
        ...
        SET build_id = :build_id,
            build_requirement_id = :build_requirement_id,
...
bool BuildAllocationRepository::removeAllForRequirement(int buildRequirementId)
{
    if (buildRequirementId <= 0)
        return false;
    QSqlQuery query(DatabaseManager::instance().database());
    ...
    query.bindValue(":id", buildRequirementId);
    if (!query.exec()) {
        qCritical() << "Unable to remove requirement allocations:" << query.lastError().text();
        return false;
    }
    return true;
}

int BuildAllocationRepository::totalAllocatedForRequirement(int buildRequirementId) const
{
    if (buildRequirementId <= 0)
        return 0;
    QSqlQuery query(DatabaseManager::instance().database());
    ...
    if (!query.exec() || !query.next()) {
        if (query.lastError().isValid())
            qCritical() << "Unable to calculate requirement allocation:" << query.lastError().text();
        return 0;
    }
    return query.value(0).toInt();
}

int BuildAllocationRepository::totalAllocatedForPartColor(int workspaceId, int partId, int colorId) const
{
    ...
    query.prepare(R"(
        ...
        INNER JOIN build b ON b.id = ba.build_id
...
int BuildAllocationRepository::totalAllocatedForPartColorForBuild(int buildId, int partId, int colorId) const
{
    ...
    query.prepare(R"(
        ...
        WHERE build_id = :build_id AND part_id = :part_id AND color_id = :color_id
    )");
    query.bindValue(":build_id", buildId);
    query.bindValue(":part_id", partId);
    query.bindValue(":color_id", colorId);
                                                                        ...
                                                                        int buildId) const
{
    ...
    if (!query.exec()) {
        qCritical() << "Unable to calculate Build allocation for inventory record:"
                    << query.lastError().text();
        return 0;
    }
...
BuildAllocation BuildAllocationRepository::allocationFromQuery(const QSqlQuery& query) const
{
    ...
    allocation.setBuildRequirementId(query.value("build_requirement_id").toInt());
    ...
    allocation.setCreatedUtc(QDateTime::fromString(query.value("created_utc").toString(), Qt::ISODateWithMs));
    allocation.setModifiedUtc(QDateTime::fromString(query.value("modified_utc").toString(), Qt::ISODateWithMs));
    return allocation;
}
