...
{
    QDialog d(this);d.setWindowTitle(tr("Export 3D Model"));auto*l=new QFormLayout(&d);QComboBox geometry(&d),format(&d),profiles(&d);geometry.addItem(tr("Source Mesh"),0);if(m_preparedMesh)geometry.addItem(tr("Prepared Mesh — Nominal / Ready for Printing"),1);const bool manufacturingSupported=m_preparedMesh&&m_request.partNumber==QStringLiteral("3700");if(manufacturingSupported)geometry.addItem(tr("ManufacturingMesh — Explicit Fit Profile"),2);if(m_preparedMesh)geometry.setCurrentIndex(1);format.addItems({tr("OBJ"),tr("Binary STL"),tr("3MF")});profiles.addItem(tr("Select a compatible Verified Fit Profile..."),QString());if(manufacturingSupported){PrintGeometry::FitCalibrationLibrary library;for(const auto&summary:library.profiles()){if(!summary.compatible)continue;PrintGeometry::FitProfile profile;QString error;if(library.loadProfile(summary.identity,&profile,&error)&&PrintGeometry::ManufacturingMeshService::compatibleCorrection(profile,PrintGeometry::FitPrintedOrientation::FeatureAxisPerpendicularToBuildPlate))profiles.addItem(summary.name,summary.identity);}}l->addRow(tr("Geometry:"),&geometry);l->addRow(tr("Fit Profile:"),&profiles);l->addRow(tr("Format:"),&format);l->addRow(tr("Scale:"),new QLabel(QString::number(m_state.scalePercent(),'f',2)+QStringLiteral("%"),&d));auto*note=new QLabel(&d);note->setWordWrap(true);l->addRow(note);QDialogButtonBox buttons(QDialogButtonBox::Save|QDialogButtonBox::Cancel,&d);l->addRow(&buttons);const auto updateChoice=[&]{const bool manufacturing=geometry.currentData().toInt()==2;profiles.setEnabled(manufacturing);buttons.button(QDialogButtonBox::Save)->setEnabled(!manufacturing||!profiles.currentData().toString().isEmpty());if(manufacturing)note->setText(profiles.count()>1?tr("A separate compensated ManufacturingMesh will be generated with the explicitly selected profile. Nominal Prepared geometry remains unchanged."):tr("No compatible managed Verified Fit Profile is available. ManufacturingMesh export cannot proceed."));else note->setText(m_preparedMesh?tr("Prepared Mesh exports nominal validated geometry. Source remains available."):tr("Source export is not a BrickSuite printability guarantee. Your slicer may need to repair it."));};connect(&geometry,qOverload<int>(&QComboBox::currentIndexChanged),&d,[&](int){updateChoice();});connect(&profiles,qOverload<int>(&QComboBox::currentIndexChanged),&d,[&](int){updateChoice();});connect(&buttons,&QDialogButtonBox::accepted,&d,&QDialog::accept);connect(&buttons,&QDialogButtonBox::rejected,&d,&QDialog::reject);updateChoice();if(d.exec()!=QDialog::Accepted)return;
...
{
    if(!m_preparedMesh)return;m_export->setEnabled(false);m_status->setText(tr("Generating profile-driven ManufacturingMesh for export..."));const auto source=m_loadResult;const auto prepared=*m_preparedMesh;auto*watcher=new QFutureWatcher<PrintGeometry::ManufacturingMeshResult>(this);QPointer<LDrawModelViewerWindow>self(this);connect(watcher,&QFutureWatcher<PrintGeometry::ManufacturingMeshResult>::finished,this,[self,watcher,profile,stl,threeMf,path,ldrawId,scale,color]{const auto result=watcher->result();watcher->deleteLater();if(!self)return;self->m_export->setEnabled(true);if(!result.ok()||!result.manufacturingMesh){QMessageBox::critical(self,self->tr("Export ManufacturingMesh"),result.diagnostic);self->m_status->setText(self->tr("ManufacturingMesh export was rejected; no nominal geometry was exported."));return;}const auto&manufacturing=*result.manufacturingMesh;QString failure;if(threeMf){ThreeMfWriter::Options options;options.uniformScale=scale;options.objectName=QStringLiteral("%1 ManufacturingMesh %2").arg(manufacturing.partReference,manufacturing.identity);options.partIdentity=manufacturing.partReference;options.modelColor=color;if(!ThreeMfWriter::write(manufacturing.mesh,path,options,&failure)){QMessageBox::critical(self,self->tr("Export ManufacturingMesh"),failure);return;}}else if(stl){if(!BinaryStlWriter::write(manufacturing.mesh,path,scale,&failure)){QMessageBox::critical(self,self->tr("Export ManufacturingMesh"),failure);return;}}else{LDrawObjWriter::Options options;options.uniformScale=scale;options.partNumber=manufacturing.partReference;options.ldrawId=ldrawId;options.geometryLabel=QStringLiteral("ManufacturingMesh / Fit Profile %1 / %2").arg(profile.name,profile.profileIdentity);LDrawGeometry::Error error;if(!LDrawObjWriter::write(manufacturing.mesh,path,options,&error)){QMessageBox::critical(self,self->tr("Export ManufacturingMesh"),error.message);return;}}self->m_status->setText(self->tr("ManufacturingMesh exported with %1 (%2 mm correction; resulting diameter %3 mm).").arg(profile.name).arg(manufacturing.diameterCorrectionMillimetres,0,'f',3).arg(manufacturing.manufacturingDiameterMillimetres,0,'f',3));});watcher->setFuture(QtConcurrent::run([source,prepared,profile]{return PrintGeometry::ManufacturingMeshService().generate(source,prepared,profile,PrintGeometry::FitPrintedOrientation::FeatureAxisPerpendicularToBuildPlate);}));
}
