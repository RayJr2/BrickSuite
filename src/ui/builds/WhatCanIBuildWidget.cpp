      ...
      m_remoteCollection(remoteCollection)
{
    ...
    connect(m_resultsTable,&QTableWidget::cellDoubleClicked,this,[this](int row,int){auto*i=m_resultsTable->item(row,1);if(i)showDetails(i->data(Qt::UserRole).toInt());});
    ...
    if(m_remoteReads){m_modeCombo->setCurrentIndex(1);m_modeCombo->setItemData(0,0,Qt::UserRole-1);m_modeCombo->setItemData(0,QStringLiteral("Requires Host-authoritative Inventory availability."),Qt::ToolTipRole);m_modeCombo->setToolTip(QStringLiteral("From My Inventory requires Host-authoritative Inventory availability. Using Selected Parts remains available."));}
...
void WhatCanIBuildWidget::loadThemes(){m_themeCombo->addItem(QStringLiteral("All Themes"),0);for(const auto&t:ThemeCatalogRepository().activeSetFilterHierarchy())m_themeCombo->addItem(t.qualifiedName,t.id);}
...
void WhatCanIBuildWidget::invalidateOperationalData(){if(!m_inventoryMode)return;m_inventoryService->cancel();m_searchOutstanding=false;m_inventoryResults.clear();m_includeCollection->setEnabled(true);m_page=0;m_stale=true;m_hasSearched=false;showPage();m_status->setText(QStringLiteral("Inventory, Builds, Collection, or Workspace data changed — run discovery again."));}

void WhatCanIBuildWidget::modeChanged(){m_inventoryMode=m_modeCombo->currentIndex()==0&&!m_remoteReads;m_modeStack->setCurrentIndex(m_inventoryMode?0:1);m_results.clear();m_inventoryResults.clear();m_page=0;m_hasSearched=false;m_stale=false;showPage();m_status->setText(m_inventoryMode?QStringLiteral("Choose a minimum buildability and start discovery."):QStringLiteral("Select at least one Part to begin."));updateControls();}
...
void WhatCanIBuildWidget::showPage()
{
    ...
    if(m_inventoryMode){m_resultsTable->setColumnCount(11);m_resultsTable->setHorizontalHeaderLabels({QStringLiteral("Image"),QStringLiteral("Set #"),QStringLiteral("Name"),QStringLiteral("Year"),QStringLiteral("Theme"),QStringLiteral("Parts"),QStringLiteral("Loose"),QStringLiteral("With Collection"),QStringLiteral("Missing"),QStringLiteral("Sources"),QStringLiteral("Actions")});m_resultsTable->horizontalHeader()->setSectionResizeMode(2,QHeaderView::Stretch);for(int c:{0,1,3,4,5,6,7,8,9,10})m_resultsTable->horizontalHeader()->setSectionResizeMode(c,QHeaderView::ResizeToContents);
        for(int i=begin;i<end;++i){const auto&x=m_inventoryResults.at(i);int r=m_resultsTable->rowCount();m_resultsTable->insertRow(r);auto*image=new QTableWidgetItem;m_resultsTable->setItem(r,0,image);auto*num=new QTableWidgetItem(x.setNumber);num->setData(Qt::UserRole,x.setCatalogId);m_resultsTable->setItem(r,1,num);m_resultsTable->setItem(r,2,new QTableWidgetItem(x.name));m_resultsTable->setItem(r,3,new QTableWidgetItem(QString::number(x.year)));m_resultsTable->setItem(r,4,new QTableWidgetItem(x.themeName));m_resultsTable->setItem(r,5,new QTableWidgetItem(QStringLiteral("%1 requirements / %2 pieces").arg(x.totalRequirements).arg(x.totalQuantity)));m_resultsTable->setItem(r,6,new QTableWidgetItem(QStringLiteral("%1% pieces; %2/%3 requirements").arg(x.loosePercent()).arg(x.looseSatisfiedRequirements).arg(x.totalRequirements)));m_resultsTable->setItem(r,7,new QTableWidgetItem(QStringLiteral("%1% pieces; %2/%3 requirements").arg(x.advisoryPercent()).arg(x.advisorySatisfiedRequirements).arg(x.totalRequirements)));m_resultsTable->setItem(r,8,new QTableWidgetItem(QString::number(x.missingQuantity)));auto*source=new QPushButton(x.sources.isEmpty()?QStringLiteral("Loose only"):QStringLiteral("%1 item(s)").arg(x.sources.size()),m_resultsTable);source->setEnabled(!x.sources.isEmpty());connect(source,&QPushButton::clicked,this,[this,x]{showSources(x);});m_resultsTable->setCellWidget(r,9,source);auto*a=new QComboBox(m_resultsTable);a->addItem(QStringLiteral("Actions..."));a->addItem(QStringLiteral("View Set Details"),QStringLiteral("details"));a->addItem(QStringLiteral("View Missing Parts"),QStringLiteral("missing"));a->addItem(QStringLiteral("Create Build"),QStringLiteral("build"));connect(a,&QComboBox::currentIndexChanged,this,[this,a,x](int n){if(n<=0)return;const QString action=a->itemData(n).toString();a->setCurrentIndex(0);if(action=="details")showDetails(x.setCatalogId);else if(action=="missing")showMissingParts(x);else{if(x.usesCollection()&&QMessageBox::warning(this,QStringLiteral("Advisory Collection Sources"),QStringLiteral("This result uses Collection items only as an advisory source. Creating a Build does not reserve or consume those items. Continue?"),QMessageBox::Yes|QMessageBox::No)!=QMessageBox::Yes)return;emit createBuildRequested(x.setCatalogId,QStringLiteral("Stock"));}});m_resultsTable->setCellWidget(r,10,a);if(!x.imageUrl.isEmpty())m_images->requestSetImage(x.setNumber,x.imageUrl);} }
...
void WhatCanIBuildWidget::showMissingParts(const InventoryBuildabilitySetResult&x){QDialog dialog(this);dialog.setWindowTitle(QStringLiteral("Missing Parts — %1").arg(x.setNumber));dialog.resize(760,420);auto*l=new QVBoxLayout(&dialog);auto*t=new QTableWidget(&dialog);t->setColumnCount(7);t->setHorizontalHeaderLabels({"Part #","Name","Color","Required","Loose","Collection","Missing"});for(const auto&r:x.requirements)if(r.missing>0){int row=t->rowCount();t->insertRow(row);t->setItem(row,0,new QTableWidgetItem(r.partNumber));t->setItem(row,1,new QTableWidgetItem(r.partName));t->setItem(row,2,new QTableWidgetItem(r.colorName));t->setItem(row,3,new QTableWidgetItem(QString::number(r.required)));t->setItem(row,4,new QTableWidgetItem(QString::number(r.looseUsed)));t->setItem(row,5,new QTableWidgetItem(QString::number(r.collectionUsed)));t->setItem(row,6,new QTableWidgetItem(QString::number(r.missing)));}t->horizontalHeader()->setSectionResizeMode(1,QHeaderView::Stretch);l->addWidget(t);auto*b=new QDialogButtonBox(QDialogButtonBox::Close,&dialog);connect(b,&QDialogButtonBox::rejected,&dialog,&QDialog::reject);l->addWidget(b);dialog.exec();}
void WhatCanIBuildWidget::showSources(const InventoryBuildabilitySetResult&x){QStringList rows;for(const auto&s:x.sources)rows<<QStringLiteral("%1 (%2) — %3 piece(s)").arg(s.label,s.state).arg(s.piecesUsed);QMessageBox::information(this,QStringLiteral("Advisory Collection Sources"),rows.join('\n'));}
