      ...
      m_remoteCollection(remoteCollection)
{
    ...
    auto* intro = new QLabel(QStringLiteral("Find Sets in the local catalog that use selected Parts. Discovery does not reserve Inventory or contact providers."), this);
    ...
    m_criteriaTable->setEditTriggers(QAbstractItemView::NoEditTriggers); layout->addWidget(m_criteriaTable);
...
void WhatCanIBuildWidget::markStale(){const bool cancelled=m_searchOutstanding;if(cancelled){m_service->cancel();m_searchOutstanding=false;}if(m_hasSearched||cancelled){m_stale=true;m_status->setText(QStringLiteral("Search criteria changed — select Find Matching Sets to refresh results."));emit statusMessageRequested(QStringLiteral("Part Usage search criteria changed."),5000);}updateControls();}
void WhatCanIBuildWidget::invalidateCatalog(){m_service->cancel();m_searchOutstanding=false;m_results.clear();m_page=0;m_stale=true;m_hasSearched=false;showPage();m_status->setText(QStringLiteral("Local catalog data changed — search again."));}
...
{
    const int size=m_pageSizeCombo?m_pageSizeCombo->currentData().toInt():100;const int pages=qMax(1,(m_results.size()+size-1)/size);if(m_page>=pages)m_page=pages-1;
    m_resultsTable->setRowCount(0);const int begin=m_page*size,end=qMin(begin+size,m_results.size());
...
void WhatCanIBuildWidget::updateControls(){m_addButton->setEnabled(m_criteria.size()<20&&m_resolvedPart.has_value());m_searchButton->setEnabled(!m_searchOutstanding&&!m_criteria.isEmpty());m_addButton->setToolTip(m_criteria.size()>=20?QStringLiteral("A search can contain at most 20 selected Part criteria."):QString());}
