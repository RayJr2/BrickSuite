...
void InventoryExportDialog::exportCsv(){const auto p=InventoryExportService::project(m_rows,configuration());auto&dirs=SessionFileDialogDirectoryService::instance();const QString name=QFileDialog::getSaveFileName(this,"Export Inventory CSV",dirs.initialFilePath(FileDialogDirectoryCategory::SaveExport,"BrickSuite_Inventory.csv"),"CSV Files (*.csv)");if(name.isEmpty())return;dirs.rememberSelectedFile(FileDialogDirectoryCategory::SaveExport,name);const auto result=InventoryCsvWriter::write(name,p);if(!result.success)QMessageBox::critical(this,"Export Inventory",result.message);else{QMessageBox::information(this,"Export Inventory",result.message);accept();}}
