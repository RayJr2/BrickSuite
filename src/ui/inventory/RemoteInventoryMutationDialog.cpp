...
RemoteInventoryMutationDialog::RemoteInventoryMutationDialog(const QString& operation,int workspaceId,
    ...
    SessionStorageSelectionService* sessionStorageSelections,
    const QString& remoteAuthority,
    const QList<RemoteReadDto::StorageSummary>& remoteStorage,QWidget* parent)
    :QDialog(parent),m_operation(operation),m_workspaceId(workspaceId),m_service(service),
     m_sessionStorageSelections(sessionStorageSelections),m_remoteAuthority(remoteAuthority),
     m_remoteStorage(remoteStorage),m_hostManufacturerNames(hostManufacturerNames),
     m_detail(std::move(detail)){initialize(storagePaths);}
...
void RemoteInventoryMutationDialog::initialize(const QHash<int,QString>& paths)
{
    ...
    if(move){i=m_storage->findData(storageId);if(i>=0)m_storage->removeItem(i);if(m_sessionStorageSelections){const auto validator=[locations=m_remoteStorage](int workspace,int id,int excluded){return RemoteStorageDestination::isValidInventoryDestination(locations,workspace,id,excluded);};const int remembered=m_sessionStorageSelections->rememberedDestination(m_remoteAuthority,m_workspaceId,validator,storageId);i=m_storage->findData(remembered);if(i>=0)m_storage->setCurrentIndex(i);}}
