...
int main(int argc, char** argv)
{
    ...
    ok &= require(DatabaseSchema::CurrentSchemaVersion == 32, "schema is version 32");
    ...
    ok &= require(createDatabase(source, 32), "create current-schema source database");
    ...
    ok &= require(successfulPath.contains("/v32/") || successfulPath.contains("\\v32\\"),
                  "v32 derived from root");
    ...
    const QString healthGateDirectory = AutomaticBackupPolicy::versionDirectory(healthGateRoot, 32);
    ...
    const QString existingBackup = QDir(healthGateDirectory).filePath(
        "BrickSuite_AutoBackup_v32_2020-01-01_000000.db");
    ...
    const QString retentionDirectory = AutomaticBackupPolicy::versionDirectory(retentionRoot, 32);
    ...
    const QString oldTwo = QDir(retentionDirectory).filePath(
        "BrickSuite_AutoBackup_v32_2020-01-02_000000.db");
    ...
    const QStringList retained = QDir(retentionDirectory).entryList(
        {"BrickSuite_AutoBackup_v32_*.db"}, QDir::Files, QDir::Name);
    ...
    const QString warningDirectory = AutomaticBackupPolicy::versionDirectory(warningRoot, 32);
    ...
    QDir versionDir(AutomaticBackupPolicy::versionDirectory(root, 32));
