...
bool hingeProductionProof(const QStringList& args) {
    bool ok=true;
    const int libraryAt=args.indexOf(QStringLiteral("--ldraw"));
    if(libraryAt<0||libraryAt+1>=args.size())return check(false,"hinge production proof needs --ldraw");
    auto selected=profile();selected.corrections.clear();
    FitProfileCorrection correction;
    correction.featureFamily="PinBarrelHinge";correction.featureRole="male";
    correction.printedOrientation="feature-axis-parallel-to-build-plate";
    correction.semanticContractVersion="official-ldraw-3937-3938-rotating-pair-v1";
    correction.semantics="male-pin-barrel-hinge-pin-od";
    correction.correctionContractVersion="male-pin-barrel-hinge-pin-od-v1";
    correction.regeneratorAlgorithmVersion=FitCalibrationLibrary::currentRegeneratorAlgorithmVersion();
    correction.calibrationArtifactIdentity="synthetic-hinge-test";
    correction.valueMillimetres=-.10;
    selected.corrections.push_back(correction);
    const int profileAt=args.indexOf(QStringLiteral("--hinge-profile"));
    if(profileAt>=0&&profileAt+1<args.size()){
        QFile file(args[profileAt+1]);ok&=check(file.open(QIODevice::ReadOnly),"managed hinge profile opens read-only");
        if(file.isOpen()){
            FitProfile parsed;QString error;
            const auto document=QJsonDocument::fromJson(file.readAll());
            ok&=check(document.isObject()&&FitProfileJson::fromJson(document.object(),&parsed,&error),
                "managed hinge profile parses: "+error);
            if(!parsed.corrections.isEmpty())selected=parsed;
        }
    }
    ok&=check(FitCalibrationLibrary::profileCompatibility(selected),"Verified hinge profile is compatible");
    const auto* chosen=ManufacturingMeshService::compatibleCorrections(selected,
        FitPrintedOrientation::FeatureAxisParallelToBuildPlate).pinBarrelHingeDiameter;
    ok&=check(chosen&&std::abs(chosen->valueMillimetres+.10)<1e-9,
        "Verified parallel profile selects the physical -0.10 mm hinge correction");
    const auto male=LDrawLibraryService::loadPart(args[libraryAt+1],QStringLiteral("3938"));
    const auto barrel=LDrawLibraryService::loadPart(args[libraryAt+1],QStringLiteral("3937"));
    ok&=check(male.ok()&&barrel.ok(),"real 3938 and 3937 source parts load");
    if(!male.ok()||!barrel.ok())return false;
    const auto pins=PinBarrelHingeSemantic::recognize(male);
    const auto receivers=PinBarrelHingeSemantic::recognize(barrel);
    ok&=check(pins.size()==2&&receivers.size()==2&&pins.front().role==FunctionalInterfaceRole::Male&&
        receivers.front().role==FunctionalInterfaceRole::Female,"two certified male pins and two nominal receiving barrels");
    PrintPreparationRequest request;request.partReference="3938";request.ldrawIdentity="parts/3938.dat";
    request.libraryAuthority=args[libraryAt+1];request.loadResult=male;
    LDrawPrintPreparationService preparation;
    const auto preparedResult=preparation.prepare(request);
    ok&=check(preparedResult.ready(),"3938 nominal preparation: "+preparedResult.diagnostic);
    if(!preparedResult.ready())return false;
    const auto& prepared=*preparedResult.preparedMesh;
    const auto before=prepared.mesh;
    const auto sourceBefore=male.mesh.triangles;
    PrintOrientation orientation;
    const auto resolved=AutoFitProfileResolver::resolve(true,"3938",{selected},male,orientation);
    ok&=check(resolved.resolved()&&resolved.profile.profileIdentity==selected.profileIdentity,
        "3938 Auto Fit resolves the Verified parallel profile");
    ok&=check(!ManufacturingMeshService::hasApplicableCorrection(selected,barrel,orientation),
        "3937 receiving barrel has no male-pin correction");
    const auto generated=ManufacturingMeshService().generate(male,prepared,selected,orientation);
    ok&=check(generated.ok(),"3938 ManufacturingMesh: "+generated.diagnostic);
    if(!generated.ok())return false;
    const auto& mesh=*generated.manufacturingMesh;
    ok&=check(mesh.featureIdentities.size()>=2&&
        std::abs(mesh.nominalDiameterMillimetres-3.2)<1e-9&&
        std::abs(mesh.diameterCorrectionMillimetres+.10)<1e-9&&
        std::abs(mesh.manufacturingDiameterMillimetres-3.1)<1e-9,
        "both hinge pins receive profile-driven 3.10 mm OD");
    ok&=check(same(prepared.mesh,before)&&sameSource(male.mesh.triangles,sourceBefore)&&
        validatePreparedMesh(mesh.analysis).ok(),"Source and Prepared remain nominal; ManufacturingMesh is valid");
    for(const auto& pin:pins){
        int moved=0,protectedVertices=0;
        double correctedCentralRadius=0;
        for(std::size_t i=0;i<before.vertices.size();++i){
            const auto& a=before.vertices[i];const auto& b=mesh.mesh.vertices[i];
            const Point relative{a.x-pin.frame.origin.x,a.y-pin.frame.origin.y,a.z-pin.frame.origin.z};
            const double axial=relative.x*pin.frame.axis.x+relative.y*pin.frame.axis.y+relative.z*pin.frame.axis.z;
            const Point radial{relative.x-axial*pin.frame.axis.x,relative.y-axial*pin.frame.axis.y,
                               relative.z-axial*pin.frame.axis.z};
            const double radius=std::sqrt(radial.x*radial.x+radial.y*radial.y+radial.z*radial.z);
            if(axial<-.02||axial>1.62)continue;
            if(radius<.95){++protectedVertices;ok&=check(a.x==b.x&&a.y==b.y&&a.z==b.z,
                "the 1.60 mm hinge bore remains nominal");}
            if(radius>1.45&&radius<1.7&&(a.x!=b.x||a.y!=b.y||a.z!=b.z))++moved;
            if(axial>.4&&axial<1.2&&radius>1.45&&radius<1.7){
                const Point changed{b.x-pin.frame.origin.x,b.y-pin.frame.origin.y,b.z-pin.frame.origin.z};
                const double newAxial=changed.x*pin.frame.axis.x+changed.y*pin.frame.axis.y+changed.z*pin.frame.axis.z;
                const Point newRadial{changed.x-newAxial*pin.frame.axis.x,
                    changed.y-newAxial*pin.frame.axis.y,changed.z-newAxial*pin.frame.axis.z};
                correctedCentralRadius=std::max(correctedCentralRadius,
                    std::sqrt(newRadial.x*newRadial.x+newRadial.y*newRadial.y+newRadial.z*newRadial.z));
            }
        }
        ok&=check(moved>=24&&protectedVertices>=8&&std::abs(2*correctedCentralRadius-3.10)<.025,
            "each pin reaches approximately 3.10 mm at its contact belt while its inner bore is protected");
    }
    const auto repeated=ManufacturingMeshService().generate(male,prepared,selected,orientation);
    ok&=check(repeated.ok()&&repeated.manufacturingMesh->identity==mesh.identity&&
        same(repeated.manufacturingMesh->mesh,mesh.mesh),"3938 ManufacturingMesh is deterministic");
    auto noHinge=selected;noHinge.corrections.erase(std::remove_if(noHinge.corrections.begin(),noHinge.corrections.end(),
        [](const FitProfileCorrection& c){return c.featureFamily==QStringLiteral("PinBarrelHinge");}),
        noHinge.corrections.end());
    ok&=check(!ManufacturingMeshService::hasApplicableCorrection(noHinge,male,orientation)||
        ManufacturingMeshService::compatibleCorrections(noHinge,FitPrintedOrientation::FeatureAxisParallelToBuildPlate).pinBarrelHingeDiameter==nullptr,
        "missing hinge evidence does not select a hinge correction");
    PrintOrientation perpendicular;perpendicular.rotate(PrintOrientation::Rotation::YPositive);
    ok&=check(!ManufacturingMeshService::compatibleCorrections(selected,
        FitPrintedOrientation::FeatureAxisPerpendicularToBuildPlate).pinBarrelHingeDiameter,
        "perpendicular evidence cannot substitute for Verified parallel hinge evidence");
    auto hingeOnly=selected;hingeOnly.corrections={*chosen};
    ok&=check(ManufacturingMeshService::transformedOrientation(pins.front(),perpendicular)==
        FitPrintedOrientation::FeatureAxisPerpendicularToBuildPlate&&
        !ManufacturingMeshService::hasApplicableCorrection(hingeOnly,male,perpendicular)&&
        ManufacturingMeshService().generate(male,prepared,hingeOnly,perpendicular).error==
            ManufacturingMeshError::MissingCorrection,
        "rotated hinge without perpendicular evidence remains nominal");
    const auto withoutHinge=ManufacturingMeshService().generate(male,prepared,noHinge,orientation);
    bool hingeStayedNominal=true;
    if(withoutHinge.ok())for(const auto& pin:pins){
        hingeStayedNominal&=!withoutHinge.manufacturingMesh->featureIdentities.contains(pin.stableIdentity);
        for(std::size_t i=0;i<before.vertices.size();++i){
            const auto& a=before.vertices[i];const auto& b=withoutHinge.manufacturingMesh->mesh.vertices[i];
            const Point d{a.x-pin.frame.origin.x,a.y-pin.frame.origin.y,a.z-pin.frame.origin.z};
            const double axial=d.x*pin.frame.axis.x+d.y*pin.frame.axis.y+d.z*pin.frame.axis.z;
            const Point r{d.x-axial*pin.frame.axis.x,d.y-axial*pin.frame.axis.y,d.z-axial*pin.frame.axis.z};
            const double radius=std::sqrt(r.x*r.x+r.y*r.y+r.z*r.z);
            if(axial>.4&&axial<1.2&&radius>1.5&&radius<1.7)
                hingeStayedNominal&=a.x==b.x&&a.y==b.y&&a.z==b.z;
        }
    }
    ok&=check(hingeStayedNominal&&
        (withoutHinge.ok()||withoutHinge.error==ManufacturingMeshError::IncompatibleProfile),
        "missing hinge evidence leaves both pins nominal while independent stud evidence may still apply");
    auto unverified=hingeOnly;unverified.verificationState=FitEvidenceState::Draft;
    ok&=check(!ManufacturingMeshService::hasApplicableCorrection(unverified,male,orientation)&&
        ManufacturingMeshService().generate(male,prepared,unverified,orientation).error==
            ManufacturingMeshError::IncompatibleProfile,
        "unverified hinge evidence remains nominal");
    for(const QString& negative:{QStringLiteral("4275a"),QStringLiteral("4276a"),QStringLiteral("30364")}){
        const auto other=LDrawLibraryService::loadPart(args[libraryAt+1],negative);
        ok&=check(other.ok()&&PinBarrelHingeSemantic::recognize(other).isEmpty()&&
            !ManufacturingMeshService::hasApplicableCorrection(hingeOnly,other,orientation),
            negative+" finger/click hinge remains outside the 3937/3938 contract");
    }
    const int outputAt=args.indexOf(QStringLiteral("--hinge-output"));
    if(outputAt>=0&&outputAt+1<args.size()){
        QString error;const QString path=args[outputAt+1];
        ok&=check(ManufacturingMeshDiagnosticExporter::writeThreeMf(mesh,path,1.0,QColor("#A0A5A9"),&error),
            "3938 ManufacturingMesh proof export: "+error);
        if(QFileInfo::exists(path)){
            ...
            ok&=check(objects->MoveNext()&&objects->GetCurrentMeshObject()->GetTriangleCount()==mesh.mesh.faces.size(),
                "3938 proof 3MF reopens with corrected mesh triangles");
        }
    }
    QTextStream(stdout)<<"hingeProfile="<<selected.profileIdentity<<" correction="<<chosen->valueMillimetres
        <<" pinOD="<<mesh.manufacturingDiameterMillimetres<<" protectedBore=1.60"
        <<" correctedFeatures="<<mesh.featureIdentities.size()<<" diagnostic="<<generated.diagnostic<<Qt::endl;
    return ok;
}
bool interleavedProductionProof(const QStringList& args) {
    const int libraryAt=args.indexOf(QStringLiteral("--ldraw"));
    if(libraryAt<0||libraryAt+1>=args.size())return check(false,"interleaved proof requires --ldraw");
    bool ok=true;
    for(const QString& part:{QStringLiteral("4275a"),QStringLiteral("4276a")}) {
        const auto source=LDrawLibraryService::loadPart(args[libraryAt+1],part);
        ok&=check(source.ok(),part+" source loads");if(!source.ok())continue;
        const auto features=InterleavedFingerHingeSemantic::recognize(source);
        ok&=check(features.size()==1,part+" certified hinge recognized");
        PrintPreparationRequest request;request.partReference=part;
        ...
        const auto result=LDrawPrintPreparationService().prepare(request);
        ok&=check(result.ready(),part+" nominal preparation succeeds: "+result.diagnostic);
        if(!result.ready())continue;
        const auto& prepared=*result.preparedMesh;
        ok&=check(std::any_of(prepared.functionalFeatures.cbegin(),prepared.functionalFeatures.cend(),
            [](const FunctionalFeature& feature){return feature.family==FunctionalInterfaceFamily::InterleavedFingerHinge;}),
            part+" retains hinge semantic ownership");
        const auto before=prepared.mesh;
        const auto sourceBefore=source.mesh.triangles;
        PrintOrientation orientation;
        const auto nominal=ManufacturingMeshService().generate(source,prepared,FitProfile{},orientation);
        if(part==QStringLiteral("4275a")) {
            ok&=check(nominal.ok()&&same(nominal.manufacturingMesh->mesh,before)&&
                nominal.diagnostic.contains(QStringLiteral("nominal/unverified"))&&
                nominal.manufacturingMesh->fitProfileIdentity.isEmpty(),
                "unverified male hinge exports nominal geometry without claiming Verified evidence: "+nominal.diagnostic);
            if(nominal.ok()) {
                const auto again=ManufacturingMeshService().generate(source,prepared,FitProfile{},orientation);
                ok&=check(again.ok()&&again.manufacturingMesh->identity==nominal.manufacturingMesh->identity,
                    "nominal hinge fallback is deterministic");
            }
            auto verified=profile();verified.corrections.clear();
            FitProfileCorrection correction;
            correction.featureFamily="InterleavedFingerHinge";correction.featureRole="male";
            correction.printedOrientation="feature-axis-parallel-to-build-plate";
            correction.semanticContractVersion="official-ldraw-h1-h2-interleaved-finger-v1";
            correction.semantics="male-interleaved-finger-contact-bump-protrusion";
            correction.correctionContractVersion="male-interleaved-finger-contact-bump-protrusion-v1";
            correction.regeneratorAlgorithmVersion=FitCalibrationLibrary::currentRegeneratorAlgorithmVersion();
            correction.calibrationArtifactIdentity="synthetic-future-hinge-test";
            correction.valueMillimetres=.05;verified.corrections.push_back(correction);
            ok&=check(FitCalibrationLibrary::profileCompatibility(verified)&&
                ...
                AutoFitProfileResolver::resolve(true,part,{verified},source,orientation).resolved(),
                "future Verified profile becomes applicable through the normal selection path");
            ...
            ok&=check(changed.ok()&&!same(changed.manufacturingMesh->mesh,before)&&
                std::abs(changed.manufacturingMesh->manufacturingDiameterMillimetres-.35)<1e-9,
                "synthetic Verified correction adjusts certified bumps: "+changed.diagnostic);
            for(const auto turn:{PrintOrientation::Rotation::XPositive,PrintOrientation::Rotation::XNegative,
                ...
                if(ManufacturingMeshService::transformedOrientation(features.front(),rotated)!=
                ...
                ok&=check(!ManufacturingMeshService::hasApplicableCorrection(verified,source,rotated)&&
                    AutoFitProfileResolver::resolve(true,part,{verified},source,rotated).state==
                        AutoFitResolutionState::NoCompatibleProfile,
                    "parallel hinge evidence cannot substitute for perpendicular print orientation");
                const auto unsupported=ManufacturingMeshService().generate(source,prepared,verified,rotated);
                ok&=check(unsupported.ok()&&same(unsupported.manufacturingMesh->mesh,before)&&
                    unsupported.diagnostic.contains(QStringLiteral("nominal/unverified")),
                    "unsupported orientation retains nominal hinge geometry with explicit diagnostic");
                break;
            }
            const auto studs=certifiedSourceStuds(source);
            if(!studs.isEmpty()) {
                auto combined=studProfile();combined.corrections.push_back(correction);
                const auto composed=ManufacturingMeshService().generate(source,prepared,combined,orientation);
                ok&=check(composed.ok()&&composed.manufacturingMesh->featureIdentities.size()==1+studs.size()&&
                    composed.diagnostic.contains(QStringLiteral("5.150 mm"))&&
                    composed.diagnostic.contains(QStringLiteral("1.800 mm")),
                    "hinge bump and every independently applicable Standard Stud compose: "+composed.diagnostic);
            }
        }else ok&=check(nominal.ok()&&same(nominal.manufacturingMesh->mesh,before)&&
            nominal.diagnostic.contains(QStringLiteral("nominal/unverified")),
            "female mating half exports nominal/unverified geometry without claiming male correction");
        const int outputAt=args.indexOf(QStringLiteral("--interleaved-output-dir"));
        if(nominal.ok()&&outputAt>=0&&outputAt+1<args.size()) {
            const QString path=QDir(args[outputAt+1]).filePath(part+QStringLiteral("-interleaved-nominal-manufacturing.3mf"));
            QString error;
            ok&=check(ManufacturingMeshDiagnosticExporter::writeThreeMf(*nominal.manufacturingMesh,path,1.0,
                QColor("#A0A5A9"),&error),"nominal hinge 3MF export: "+error);
            if(QFileInfo::exists(path)){
                Lib3MF::CWrapper wrapper;auto model=wrapper.CreateModel();
                model->QueryReader("3mf")->ReadFromFile(path.toStdString());
                auto objects=model->GetMeshObjects();
                ok&=check(objects->MoveNext()&&objects->GetCurrentMeshObject()->GetTriangleCount()==
                    nominal.manufacturingMesh->mesh.faces.size(),"nominal hinge 3MF reopens");
            }
        }
        ok&=check(same(prepared.mesh,before)&&sameSource(source.mesh.triangles,sourceBefore),
            part+" Source and PreparedMesh remain unchanged");
    }
    return ok;
}
...
const int ballProfileAt=args.indexOf(QStringLiteral("--ball-profile"));
FitProfile actualBallProfile;
bool haveActualBallProfile=false;
if(ballProfileAt>=0 && ballProfileAt+1<args.size()) {
    QFile file(args[ballProfileAt+1]);
    ok &= check(file.open(QIODevice::ReadOnly),"actual managed Ball Joint profile opens read-only");
    if(file.isOpen()) {
        QString error;
        const auto document=QJsonDocument::fromJson(file.readAll());
        haveActualBallProfile=document.isObject() &&
            FitProfileJson::fromJson(document.object(),&actualBallProfile,&error);
        ok &= check(haveActualBallProfile,"actual managed Ball Joint profile parses: "+error);
    }
}
if(libraryAt>=0 && libraryAt+1<args.size()) {
    const auto selected=haveActualBallProfile?actualBallProfile:ballCalibrationOnly;
    for(const QString& part:{QStringLiteral("14417"),QStringLiteral("22484"),QStringLiteral("22890")}) {
        const auto ballSource=LDrawLibraryService::loadPart(args[libraryAt+1],part);
        ...
        request.ldrawIdentity=QStringLiteral("parts/%1.dat").arg(part);
        request.libraryAuthority=args[libraryAt+1];
        request.loadResult=ballSource;
        LDrawPrintPreparationService preparation;
        const auto preparedResult=preparation.prepare(request);
        const auto& sourceAnalysis=preparedResult.sourceAnalysis;
        ok &= check(preparedResult.ready(),part+" reaches nominal PreparedMesh: "+preparedResult.diagnostic+
            QStringLiteral(" [triangles=%1 components=%2 boundaries=%3 dimensions=%4,%5,%6]")
                .arg(sourceAnalysis.triangles).arg(sourceAnalysis.connectedComponents).arg(sourceAnalysis.boundaryEdges)
                .arg(sourceAnalysis.bounds.maximum.x-sourceAnalysis.bounds.minimum.x)
                .arg(sourceAnalysis.bounds.maximum.y-sourceAnalysis.bounds.minimum.y)
        ...
        ok &= check(autoFit.resolved()&&autoFit.profile.profileIdentity==selected.profileIdentity,
                    part+" Auto Fit selects the Verified Ball Joint profile");
        if(haveActualBallProfile) {
            const FitCalibrationLibrary library(QDir::cleanPath(
                QFileInfo(args[ballProfileAt+1]).absolutePath()+QStringLiteral("/..")));
            const auto managed=AutoFitProfileResolver::resolveManaged(true,part,library,ballSource,orientations[matching]);
            ok &= check(managed.resolved()&&managed.profile.profileIdentity==selected.profileIdentity,
                        part+" managed Auto Fit selects Ray's existing Verified profile");
        }
        const auto produced=ballService.generate(ballSource,prepared,selected,orientations[matching]);
        ok &= check(produced.ok(),part+" Ball Joint ManufacturingMesh succeeds: "+produced.diagnostic);
        if(produced.ok()) {
            ...
            ok &= check(correction&&mesh.fitProfileIdentity==selected.profileIdentity&&
                        ...
                        same(prepared.mesh,before)&&sameSource(ballSource.mesh.triangles,sourceBefore),
                        part+" uses selected profile while Source and Prepared remain unchanged");
            const auto repeated=ballService.generate(ballSource,prepared,selected,orientations[matching]);
            ok &= check(repeated.ok()&&repeated.manufacturingMesh->identity==mesh.identity&&
                        same(repeated.manufacturingMesh->mesh,mesh.mesh),part+" Ball Joint output is deterministic");
            int moved=0;
            bool reachesSelectedSphereRadius=false;
            bool stemAndAttachmentPreserved=true;
            for(size_t i=0;i<before.vertices.size();++i)
                if(before.vertices[i].x!=mesh.mesh.vertices[i].x||
                   before.vertices[i].y!=mesh.mesh.vertices[i].y||
                   before.vertices[i].z!=mesh.mesh.vertices[i].z) {
                    ++moved;
                    const auto& center=balls.front().frame.origin;
                    const auto& original=before.vertices[i];
                    const double originalRadius=std::sqrt((original.x-center.x)*(original.x-center.x)+
                        (original.y-center.y)*(original.y-center.y)+
                        (original.z-center.z)*(original.z-center.z));
                    stemAndAttachmentPreserved &= originalRadius<3.25;
                    const auto& changed=mesh.mesh.vertices[i];
                    const double radius=std::sqrt((changed.x-center.x)*(changed.x-center.x)+
                        (changed.y-center.y)*(changed.y-center.y)+
                        (changed.z-center.z)*(changed.z-center.z));
                    reachesSelectedSphereRadius|=std::abs(radius-mesh.manufacturingDiameterMillimetres*.5)<0.015;
                   }
            ok &= check(moved>=32&&stemAndAttachmentPreserved&&mesh.mesh.faces==before.faces,
                        part+" only the spherical envelope changes without replacing Prepared topology or distant stem/attachment vertices");
            ok &= check(reachesSelectedSphereRadius,part+" corrected spherical fit belt reaches the profile-selected radius");
            if(part==QStringLiteral("14417")||part==QStringLiteral("22890")) {
                const int outputAt=args.indexOf(QStringLiteral("--ball-output"));
                if(outputAt>=0&&outputAt+1<args.size()) {
                    QDir output(args[outputAt+1]);
                    ok &= check(output.mkpath(QStringLiteral(".")),"Ball Joint proof directory exists");
                    const QString path=output.filePath(part+QStringLiteral("-ball-joint-manufacturing.3mf"));
                    QString error;
                    ok &= check(ManufacturingMeshDiagnosticExporter::writeThreeMf(mesh,path,1.0,
                                QColor("#A0A5A9"),&error),"Ball Joint 3MF export: "+error);
                    if(QFileInfo::exists(path)) {
                        Lib3MF::CWrapper wrapper;
                        auto model=wrapper.CreateModel();
                        model->QueryReader("3mf")->ReadFromFile(path.toStdString());
                        ...
                        ok &= check(objects->MoveNext()&&
                                    objects->GetCurrentMeshObject()->GetTriangleCount()==mesh.mesh.faces.size(),
                                    "Ball Joint ManufacturingMesh 3MF reopens through lib3mf");
                        QTextStream(stdout)<<"ballManufacturing="<<path<<" correction="
                            <<mesh.diameterCorrectionMillimetres<<" diameter="
                            <<mesh.manufacturingDiameterMillimetres<<Qt::endl;
                    }
                }
            }
        }
        auto zeroProfile=ballCalibrationOnly;
        zeroProfile.corrections.front().valueMillimetres=0.0;
        const auto zero=ballService.generate(ballSource,prepared,zeroProfile,orientations[matching]);
        ok &= check(zero.ok()&&same(zero.manufacturingMesh->mesh,before),
                    part+" Verified-zero Ball Joint profile is nominal-equivalent");
        const auto synthetic=ballService.generate(ballSource,prepared,ballCalibrationOnly,orientations[matching]);
        ...
        ok &= check(!ManufacturingMeshService::hasApplicableCorrection(unverified,ballSource,orientations[matching])&&
                    AutoFitProfileResolver::resolve(true,part,{unverified},ballSource,orientations[matching]).state==AutoFitResolutionState::NoCompatibleProfile,
                    part+" unverified Ball Joint evidence remains nominal");
        auto missing=ballCalibrationOnly;
        missing.corrections.removeIf([](const FitProfileCorrection& correction){return correction.featureFamily==QStringLiteral("BallJoint");});
