...
int main(int argc,char**argv)
{
    ...
    const auto twoSources=find(40);if(!require(twoSources.sources.size()==2,"duplicate Collection copies remain distinct physical sources")||!require(twoSources.sources.first().piecesUsed==2&&twoSources.sources.last().piecesUsed==2,"composition is not multiplied by duplicate source rows"))return 1;
