...
bool verifyMigratedData(QSqlDatabase database, const Snapshot& snapshot)
{
    ...
    ok &= require(scalar(database, "SELECT version FROM schema_version").toInt() == 35,
...
bool runScenario(bool forceLateFailure)
{
    ...
    Snapshot snapshot;
    {
        ...
        if (forceLateFailure && !executeAll(database, {
                ...
                "WHEN NEW.version=35 BEGIN SELECT RAISE(ABORT,'forced late migration failure'); END"}))
            return false;
        database.close();
    }
    if (forceLateFailure) {
    ...
    manager.close();
    if (!require(manager.initialize(), "schema-35 database reopens through normal initialization"))
        return false;
    if (!verifyMigratedData(manager.database(), snapshot))
        return false;
    return true;
...
int main(int argc, char* argv[])
{
    ...
    qInfo() << "Schema 19 to schema 35 migration validation passed.";
    return 0;
}
