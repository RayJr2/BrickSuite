...
int main(int argc,char**argv)
{
    ...
    PrintOrientation displayProof;displayProof.rotate(PrintOrientation::Rotation::XPositive);const PrintGeometry::Point canonical{2,3,5};const auto exported=displayProof.map(canonical);const QVector3D ldraw(float(canonical.x),float(-canonical.z),float(canonical.y));const QVector3D displayed=displayProof.ldrawDisplayMatrix().map(ldraw);const QVector3D exportedAsLDraw(float(exported.x),float(-exported.z),float(exported.y));ok&=require(near(displayed,exportedAsLDraw),"viewer applies the authoritative export orientation through the established LDraw display basis");
